...
function isIoWorkloadFaultId(id) {
  const s = String(id || "");
  return (
    s.startsWith("threshold-io-busy-") ||
    s.startsWith("threshold-io-queue-") ||
    s.startsWith("threshold-io-latency-") ||
    s.startsWith("threshold-io-throughput-")
  );
}
...
export function processCandidatesDomainForFault(fault) {
  ...
  // Block I/O workload under IO Control — use existing disk process candidates
  // (already supported on CM.py hosts that may not expose domain=io yet).
  if (isIoWorkloadFaultId(id)) return "disk";
  if (
...
export function faultShowsProcessCandidates(fault) {
  ...
  if (isIoWorkloadFaultId(id)) return true;
...
export function candidateUsageLabel(candidate, domain) {
  ...
  if (domain === "io") {
    if (candidate?.total_MB_per_sec != null) return fmtMBps(candidate.total_MB_per_sec);
    const pct = candidate?.usage_percent;
    if (pct != null) return fmtMBps(pct);
    return "—";
  }
  const pct = candidate?.usage_percent;
  if (pct == null) return "—";
