...
function getComponentLiveMetrics(component, live, context) {
  ...
  switch (component) {
    ...
    case "IO Control":
      return [
        ["Busy", live.io_busy != null ? `${live.io_busy}%` : null],
        ["Throughput", live.io_total_mbps != null ? `${live.io_total_mbps} MB/s` : null],
        ["Device", live.io_device || null],
  ...
  switch (component) {
    ...
    case "IO Control":
      return [
        ...
        ...(inv.disks || []).slice(0, 2).map(
          (d) => `${d.device || "—"}: ${d.transport || d.type || "block"} I/O`
        ),
      ].filter(Boolean);
    default:
      return [];
