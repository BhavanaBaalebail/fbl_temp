...
CREATE TABLE IF NOT EXISTS notification_severity_state (
    fault_id TEXT NOT NULL,
    severity TEXT NOT NULL,
    first_notified_at REAL NOT NULL,
    last_notified_at REAL NOT NULL,
    snapshot_json TEXT,
    PRIMARY KEY (fault_id, severity)
);
"""

INCIDENT_ANALYSIS_DDL = """
CREATE TABLE IF NOT EXISTS incident_analysis_executions (
...
def _migrate_notification_severity_state(conn: sqlite3.Connection) -> None:
    ...
    if "notification_severity_state" not in tables or "notification_open_alerts" not in tables:
        return
    existing = conn.execute("SELECT COUNT(*) AS c FROM notification_severity_state").fetchone()["c"]
    if existing:
        return
    rows = conn.execute(
        "SELECT fault_id, first_alert_at, last_alert_at, snapshot_json FROM notification_open_alerts"
    ).fetchall()
    for row in rows:
        snapshot = {}
        if row["snapshot_json"]:
            try:
                ...
                snapshot = {}
        severity = str((snapshot or {}).get("severity") or "critical").strip().lower()
        if severity not in ("warning", "critical"):
            severity = "critical"
        conn.execute(
            """
            INSERT OR IGNORE INTO notification_severity_state (
                fault_id, severity, first_notified_at, last_notified_at, snapshot_json
            ) VALUES (?, ?, ?, ?, ?)
            """,
            (
                row["fault_id"],
...
def upsert_incident_analysis_execution(entry: dict[str, Any]) -> None:
    ...
    with _db_lock:
        ...
        try:
            ...
            conn.execute(
                """
                INSERT INTO incident_analysis_executions (
                    execution_id, incident_id, utility_id, started_at, completed_at,
                    status, exit_code, output_location, html_report_location, summary, payload
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(execution_id) DO UPDATE SET
                    ...
                    html_report_location = excluded.html_report_location,
                ...
                (
                    ...
                    entry.get("utility_id"),
                    float(entry.get("started_at") or time.time()),
                    entry.get("completed_at"),
                    entry.get("status") or "QUEUED",
                    entry.get("exit_code"),
                    entry.get("output_location"),
                    entry.get("html_report_location"),
                    entry.get("summary"),
                    payload,
                ),
            )
            ...
            logger.exception("Failed to upsert incident analysis execution")
        finally:
            conn.close()


def get_incident_analysis_execution(execution_id: str) -> Optional[dict[str, Any]]:
    with _db_lock:
        conn = _connect()
        try:
            conn.execute(INCIDENT_ANALYSIS_DDL)
            row = conn.execute(
                "SELECT * FROM incident_analysis_executions WHERE execution_id = ?",
                (execution_id,),
            ).fetchone()
            if not row:
                return None
...
def list_incident_analysis_executions(incident_id: Optional[str] = None, limit: int = 50) -> list[dict[str, Any]]:
    ...
    with _db_lock:
        ...
        try:
            ...
            if incident_id:
                rows = conn.execute(
                    "SELECT * FROM incident_analysis_executions WHERE incident_id = ? "
                    "ORDER BY started_at DESC LIMIT ?",
                    (incident_id, limit),
                ).fetchall()
            else:
                rows = conn.execute(
                    "SELECT * FROM incident_analysis_executions ORDER BY started_at DESC LIMIT ?",
                    (limit,),
                ).fetchall()
            out = []
            for row in rows:
...
def get_notification_severity_state(fault_id: str, severity: str) -> Optional[dict[str, Any]]:
    sev = _norm_alert_severity(severity)
    with _db_lock:
        conn = _connect()
        try:
            row = conn.execute(
                "SELECT fault_id, severity, first_notified_at, last_notified_at, snapshot_json "
                "FROM notification_severity_state WHERE fault_id = ? AND severity = ?",
                (fault_id, sev),
            ).fetchone()
            if not row:
                return None
...
def upsert_notification_severity_state(
    fault_id: str,
    severity: str,
    ...
    with _db_lock:
        ...
        try:
            existing = conn.execute(
                "SELECT first_notified_at FROM notification_severity_state "
                "WHERE fault_id = ? AND severity = ?",
                (fault_id, sev),
            ).fetchone()
            if existing:
                conn.execute(
                    "UPDATE notification_severity_state SET last_notified_at = ?, snapshot_json = ? "
                    "WHERE fault_id = ? AND severity = ?",
                    (now, snap_json, fault_id, sev),
                )
            else:
                conn.execute(
                    """
                    INSERT INTO notification_severity_state (
                        fault_id, severity, first_notified_at, last_notified_at, snapshot_json
                    ) VALUES (?, ?, ?, ?, ?)
                    """,
                    (fault_id, sev, now, now, snap_json),
                )
            conn.commit()
        finally:
            conn.close()
...
def clear_notification_severity_state(fault_id: str) -> None:
    with _db_lock:
        conn = _connect()
        try:
            conn.execute(
                "DELETE FROM notification_severity_state WHERE fault_id = ?",
                (fault_id,),
            )
            conn.commit()
        finally:
...
def list_notification_severity_states() -> list[dict[str, Any]]:
    with _db_lock:
        conn = _connect()
        try:
            rows = conn.execute(
                "SELECT fault_id, severity FROM notification_severity_state"
            ).fetchall()
            return [{"fault_id": r["fault_id"], "severity": r["severity"]} for r in rows]
        finally:
            conn.close()
