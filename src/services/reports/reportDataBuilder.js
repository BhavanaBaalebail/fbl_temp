...
function buildLoadShare(samples) {
  const defs = [
    { name: "CPU", values: samples.map((s) => s.cpu_usage), color: [77, 159, 255] },
    { name: "GPU", values: samples.map((s) => s.gpu_util), color: [34, 211, 238] },
    { name: "RAM", values: samples.map((s) => s.mem_usage), color: [187, 134, 252] },
    { name: "Disk", values: samples.map(peakDiskUsage), color: [245, 158, 11] },
    { name: "NIC", values: samples.map((s) => s.nic_util), color: [56, 189, 248] },
    { name: "I/O Controller", values: samples.map((s) => s.io_busy), color: [16, 185, 129] },
  ];
  const slices = defs.map((d) => {
    ...
    return {
      name: d.name,
      util: stats.avg,
      value: stats.avg != null ? stats.avg : 0,
      color: d.color,
    };
  });
