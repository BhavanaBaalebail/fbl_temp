...
/** Doughnut of relative component load share (six monitored components). */
function drawLoadShareChart(doc, loadShare, x, y, w, h) {
  const slices = (loadShare?.slices || []).slice(0, 6);
  doc.setDrawColor(...NAVY);
  doc.setLineWidth(0.35);
  doc.setFillColor(252, 253, 255);
  doc.rect(x, y, w, h, "FD");

  const cx = x + Math.min(w * 0.34, 48);
  const cy = y + h / 2;
  const r = Math.min(h * 0.38, 26);
  ...
  if (!slices.length || total <= 0) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...GRAY);
    doc.text("No utilization samples for load-share chart", x + w / 2, y + h / 2, { align: "center" });
    return;
  }

  let angle = -Math.PI / 2;
  slices.forEach((slice) => {
    const sweep = ((Number(slice.value) || 0) / total) * Math.PI * 2;
    if (sweep <= 0) return;
    const a1 = angle + sweep;
    const steps = Math.max(8, Math.round(Math.abs(sweep) / 0.08));
    const color = rgbOf(slice.color);
    doc.setFillColor(...color);
    for (let i = 0; i < steps; i += 1) {
      const t0 = angle + (sweep * i) / steps;
      const t1 = angle + (sweep * (i + 1)) / steps;
      doc.triangle(
        cx,
        cy,
        cx + r * Math.cos(t0),
  ...
  slices.forEach((slice) => {
    const color = rgbOf(slice.color);
    doc.setFillColor(...color);
    doc.rect(legendX, legendY - 2.4, 3.2, 3.2, "F");
    doc.setTextColor(...DARK);
    const share = slice.share != null ? slice.share : 0;
    const util = slice.util != null ? `${fmt(slice.util)}% util` : "no samples";
    doc.text(`${pdfSafe(slice.name)}  ${fmt(share)}%  (${util})`, legendX + 5, legendY);
    legendY += 8;
  });
}
