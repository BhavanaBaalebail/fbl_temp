...
/**
 * @param {"cpu"|"gpu"|"disk"|"nic"|"io"} domain
...
export async function fetchProcessCandidates(domain, options = {}) {
  ...
  // Prefer generic candidates; for I/O fall back to dedicated endpoint if needed.
  if (domain === "io") {
    try {
    ...
    const params = new URLSearchParams({
      min_mb_per_sec: String(minPercent),
      limit: String(limit),
    });
    const res = await fetch(`${LINUX_SERVER}/recovery/io/process_candidates?${params}`, {
      headers: { Accept: "application/json" },
    });
    if (!res.ok) {
