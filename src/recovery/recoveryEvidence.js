...
export function getTopProcessForFault(fault, metrics) {
  ...
  if (
    /threshold-io-(busy|queue|latency|throughput)-/.test(id) ||
    (component === "DISK" && /disk-busy|disk-queue|disk-latency|disk-throughput/.test(id))
  ) {
...
export function collectEvidence(fault, inventory, metrics, linkHealth) {
  ...
  const isIoWorkload =
    /threshold-io-(busy|queue|latency|throughput)-/.test(fault.id || "");

  if (component === "DISK" || fault.id?.includes("disk") || isIoWorkload) {
    ...
    if (isIoWorkload) {
      items.push({ label: "Component", value: "IO Control" });
      items.push({ label: "Fault Severity", value: fault.severity || null });
      if (fault.status) items.push({ label: "Health / Status", value: fault.status });
    }
    ...
    if (perf?.average_read_latency_ms != null) {
      items.push({ label: "Read Latency", value: fmt(perf.average_read_latency_ms, " ms") });
    }
    if (perf?.average_write_latency_ms != null) {
      items.push({ label: "Write Latency", value: fmt(perf.average_write_latency_ms, " ms") });
    }
    ...
    if (perf?.total_IOPS != null) items.push({ label: "Total IOPS", value: String(perf.total_IOPS) });
...
export function readMetricValue(path, metrics, linkHealth, fault) {
  ...
  if (path === "disk.busy_percent" || path === "io.busy_percent") {
    const perf = diskPerformanceForFault(fault, metrics);
    return perf?.busy_percent ?? null;
  }
  if (path === "disk.queue_depth" || path === "io.queue_depth") {
    const perf = diskPerformanceForFault(fault, metrics);
    return perf?.queue_depth ?? null;
  }
  if (path === "disk.average_latency_ms" || path === "io.average_latency_ms") {
    const perf = diskPerformanceForFault(fault, metrics);
    return perf?.average_latency_ms ?? null;
  }
  if (path === "disk.total_MB_per_sec" || path === "io.total_MB_per_sec") {
    const perf = diskPerformanceForFault(fault, metrics);
